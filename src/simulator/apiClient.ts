...
export class ApiClient {
  ...
  async getCommandByKey(
    key: string,
  ): Promise<{ status: number; body: ApiCommand | { error: string } }> {
    return this.request(
      "GET",
      `/api/v1/commands/by-key/${encodeURIComponent(key)}`,
    );
  }

  async getEvents(
    commandId: string,
  ): Promise<{ status: number; body: { events: ApiEvent[] } }> {
  ...
  async confirm(input: {
    ...
    status: number;
    body: { accepted: boolean; command: ApiCommand };
  }> {
    return this.request("POST", "/api/v1/gateway/confirm", input);
  }

  async scanExpired(): Promise<{
    status: number;
    body: { expired: unknown[] };
  }> {
  ...
  async withdraw(
    ...
    return this.request("POST", `/api/v1/commands/${commandId}/withdraw`, {
      reason,
      requestedBy,
    });
  }
  ...
  async supersede(
    ...
    status: number;
    body:
      | { oldCommand: ApiCommand; newCommand: ApiCommand }
      | { error: string };
  }> {
    return this.request(
      "POST",
      `/api/v1/commands/${oldCommandId}/supersede`,
      { payload, reason, requestedBy, maxAttempts },
      { "Idempotency-Key": idempotencyKey },
    );
  }
