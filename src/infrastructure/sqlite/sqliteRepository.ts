  ...
  async save(command: Command, events: CommandEvent[]): Promise<void> {
    const upsertCmd = this.db.prepare(`
      INSERT INTO commands (
        ...
        confirmation_code, device_timestamp, failure_reason, created_at, updated_at
      ) VALUES (
        @command_id, @idempotency_key, @payload, @status, @current_lease_id,
        @attempt, @max_attempts, @lease_expires_at, @gateway_id,
        @confirmation_code, @device_timestamp, @failure_reason, @created_at, @updated_at
    ...
    const saveTxn = this.db.transaction((cmds: Command[], evts: CommandEvent[]) => {
      for (const c of cmds) {
        upsertCmd.run({
...
export class SqliteUnitOfWork implements UnitOfWork {
  ...
  async transaction<T>(
    fn: (repos: { commands: CommandRepository; events: EventStore }) => Promise<T>
  ): Promise<T> {
