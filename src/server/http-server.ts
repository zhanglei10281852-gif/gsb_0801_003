...
export function createApp(config: AppConfig) {
  ...
  function maybeCrash(point: string) {
    ...
    if (point === 'afterCommit' && crashAfterNextCommit) {
      ...
      console.error('FAULT: crashing after durable commit and before HTTP response');
      process.exit(87);
    }
  }
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (method === 'POST' && path === '/v1/upstream/commands') {
        ...
        const idempotencyKey = getHeader(req,'idempotency-key') ?? (body.idempotencyKey as string);
        if (!idempotencyKey) {
          sendJson(res, 400, { error: 'IDEMPOTENCY_KEY_REQUIRED' });
          return;
        }
        if (!body.deviceId || !body.payload || typeof body.payload !== 'object') {
          sendJson(res, 400, { error: 'INVALID_REQUEST', required: ['deviceId', 'payload'] });
          return;
        }
        const result = service.submit({
          idempotencyKey,
          deviceId: body.deviceId as string,
          payload: body.payload as { type: string; params?: Record<string, unknown> },
        });
        maybeCrash('afterCommit');
        const cmd = service.getCommand(result.commandId);
        sendJson(res, result.duplicate ? 200 : 202, commandView(cmd, undefined, result.duplicate));
        return;
      }
      ...
      if (method === 'GET' && path === '/v1/upstream/commands') {
        ...
        sendJson(res, 200, { commands: service.listCommands().map((c) => commandView(c)) });
        return;
      }

      const commandMatch = path.match(/^\/v1\/(?:upstream|gateway)\/commands\/([^/]+)(?:\/(events))?$/);
      ...
      if (method === 'POST' && path === '/v1/gateway/claim') {
        const body = (await readJson(req)) as Record<string, unknown>;
        const gatewayId = getHeader(req,'x-gateway-id') ?? (body.gatewayId as string);
        if (!gatewayId) {
      ...
      const renewMatch = path.match(/^\/v1\/gateway\/commands\/([^/]+)\/renew$/);
      if (renewMatch && method === 'POST') {
        const commandId = decodeURIComponent(renewMatch[1]);
        const body = (await readJson(req)) as Record<string, unknown>;
        const gatewayId = getHeader(req,'x-gateway-id') ?? (body.gatewayId as string);
        const leaseId = getHeader(req,'x-lease-id') ?? (body.leaseId as string);
        if (!gatewayId || !leaseId) {
...
async function main() {
  ...
  app.server.listen(config.port, config.host, () => {
    ...
    console.log(`edge command service listening on http://${addr.address}:${addr.port}`);
  });
