 ...
 *
 * 恢复边界:状态机的每次决策(状态行 + 确认行 + 因果事件)在单个事务中原子落库;
 * 服务不依赖任何内存状态,崩溃重启后仅从 SQLite 恢复。
 */
import Database from 'better-sqlite3';
import { AckRecord, Attempt, Command, DomainEvent, StoredEvent } from '../domain/types';
...
CREATE INDEX IF NOT EXISTS idx_commands_pending ON commands (status, next_attempt_not_before);
...
export class SqliteRepository {
  ...
  /** 可被领取的指令:PENDING 且到达可领取时间,按提交顺序 */
  listClaimable(now: number, limit: number): Command[] {
    const rows = this.db
      .prepare(
        `SELECT * FROM commands
         WHERE status = 'PENDING' AND next_attempt_not_before <= ?
  ...
  upsertCommand(c: Command): void {
    this.db
      .prepare(
        `INSERT INTO commands (id, idempotency_key, action, params, status, execution_token,
                               attempt_count, active_attempt_id, next_attempt_not_before, created_at, updated_at)
         VALUES (@id, @idempotencyKey, @action, @params, @status, @executionToken,
  ...
  upsertAttempt(a: Attempt): void {
    this.db
      .prepare(
        `INSERT INTO attempts (id, command_id, attempt_no, gateway_id, lease_id, status,
                               leased_at, lease_expires_at, renewed_count)
         VALUES (@id, @commandId, @attemptNo, @gatewayId, @leaseId, @status,
  ...
  insertEvent(e: DomainEvent): void {
    this.db
      .prepare(
        `INSERT INTO events (command_id, attempt_id, type, data, caused_by, at)
         VALUES (@commandId, @attemptId, @type, @data, @causedBy, @at)`,
      )
