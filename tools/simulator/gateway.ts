...
export class GatewaySimulator {
  ...
  async pollOnce(
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const leased: LeasedCommand = {
        ...
        generation: body.lease.generation,
  ...
  async ack(
    cmd: LeasedCommand,
    override?: { success?: boolean; ackCode?: string },
  ) {
    const result = await this.device.process(cmd);
    const ack = await this.client.ack(
      this.gatewayId,
      cmd.commandId,
      cmd.leaseId,
      cmd.generation,
    ...
    this.acknowledgements.push({
      ...
      status: ack.status,
      reason: (ack.body as { reason?: string } | undefined)?.reason,
      body: ack.body,
    });
    return ack;
  }

  static staleGenerationAck(
    client: ApiClient,
    gatewayId: string,
    commandId: string,
    leaseId: string,
    ...
    return client.ack(gatewayId, commandId, leaseId, generation, {
      success: true,
      ackCode: "STALE_GENERATION_ACK",
    });
  }
