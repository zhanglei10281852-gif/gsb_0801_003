...
import { InvalidLeaseError, InvalidStateTransitionError } from '../../src/domain/errors.js';
...
describe('state machine — submit idempotency', () => {
  ...
  it('returns the existing command unchanged when idempotency key already exists', () => {
    ...
    clock.advance(5000);
    const second = createPendingCommand(makeSubmit(), first.command, idGen, clock);
...
describe('state machine — claim and renew', () => {
  it('transitions PENDING to CLAIMED with a new lease and increments attempt', () => {
    ...
    const decision = claimCommand(command, { gatewayId: 'gw-1', leaseDurationMs: 1000 }, idGen, clock);
  ...
  it('renews an active lease and extends expiry', () => {
    const { clock, idGen, command } = setup();
    const claimed = claimCommand(command, { gatewayId: 'gw-1', leaseDurationMs: 1000 }, idGen, clock);
    clock.advance(500);
    const renewed = renewLease(
      claimed.command,
      { commandId: claimed.command.commandId, leaseId: claimed.command.currentLeaseId!, gatewayId: 'gw-1', leaseDurationMs: 1000 },
      idGen,
      clock
    );
  ...
  it('rejects renew with wrong leaseId', () => {
    ...
    expect(() =>
      renewLease(
        claimed.command,
        { commandId: claimed.command.commandId, leaseId: 'wrong-lease', gatewayId: 'gw-1', leaseDurationMs: 1000 },
        idGen,
        clock
      )
...
describe('state machine — device confirmation', () => {
  ...
  it('rejects confirmation with stale leaseId after reclaim (old gateway late ack)', () => {
    const { clock, idGen, command } = setup();
    const first = claimCommand(command, { gatewayId: 'gw-1', leaseDurationMs: 500 }, idGen, clock);
    const firstLeaseId = first.command.currentLeaseId!;
    clock.advance(501);
    const second = claimCommand(first.command, { gatewayId: 'gw-2', leaseDurationMs: 500 }, idGen, clock);
  ...
  it('rejects confirmation on a PENDING command (lease expired and reset) without resurrecting', () => {
    const { clock, idGen, command } = setup();
    const claimed = claimCommand(command, { gatewayId: 'gw-1', leaseDurationMs: 500 }, idGen, clock);
...
describe('state machine — lease expiry and reclaim', () => {
  ...
  it('marks FAILED when the last attempt lease expires', () => {
    ...
    const second = claimCommand(expired1.decision.command, { gatewayId: 'gw-2', leaseDurationMs: 500 }, idGen, clock);
    clock.advance(501);
    ...
    expect(expired2!.decision.events.map((e) => e.eventType)).toContain('CommandFailed');
  });
  ...
  it('rejects a claim while another lease is still active', () => {
    ...
    const claimed = claimCommand(command, { gatewayId: 'gw-1', leaseDurationMs: 1000 }, idGen, clock);
    expect(() =>
      claimCommand(claimed.command, { gatewayId: 'gw-2', leaseDurationMs: 1000 }, idGen, clock)
    ).toThrow(InvalidLeaseError);
  });
  ...
  it('fails when claim would exceed maxAttempts', () => {
    ...
    expect(expired2.decision.command.status).toBe('FAILED');
    expect(() =>
      claimCommand(expired2.decision.command, { gatewayId: 'gw-3', leaseDurationMs: 500 }, idGen, clock)
...
describe('state machine — delivery report', () => {
  it('records a DeliveryReported event without changing lease state', () => {
    ...
    const reported = reportDelivery(
      claimed.command,
      { commandId: claimed.command.commandId, leaseId: claimed.command.currentLeaseId!, gatewayId: 'gw-1', deviceMessage: 'sent over Modbus' },
      idGen,
      clock
    );
...
describe('state machine — causal chain integrity', () => {
  it('produces an ordered causal event chain through a full happy path', () => {
    ...
    clock.advance(10);
    const claimed = claimCommand(submitted.command, { gatewayId: 'gw-1', leaseDurationMs: 5000 }, idGen, clock);
    clock.advance(10);
    const reported = reportDelivery(
      claimed.command,
      { commandId: claimed.command.commandId, leaseId: claimed.command.currentLeaseId!, gatewayId: 'gw-1' },
      idGen,
      clock
    );
    clock.advance(10);
